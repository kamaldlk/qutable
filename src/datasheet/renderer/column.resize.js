...
export default class ColumnResizer {
    ...
    onGripDrag = (e) => {
      ...
      //t.opt.fixed ? i === t.columnCnt - 1 ? t.tableWidth - l : t.grips[i + 1].offsetLeft - t.cellSpace - mw : Infinity;
      let max = 0;
      if(t.opt.fixed) {
        if(i === t.columnCnt - 1) {
          max = t.tableWidth - l;
        }else{
          max = t.grips[i + 1].offsetLeft - t.cellSpace - mw;
        }
      }else{
        max = Infinity;
      }
    ...
    destroy = () => {
      ...
      Reflect.deleteProperty(tt, 'extended');
      return tt.opt;
    };
    ...
    extendTable = (th) => {
      ...
      tb.cellSpace = parseInt(this.IE ? tb.cellSpacing || tb.currentStyle.borderSpacing :
        window.getComputedStyle(tb).borderSpacing.split(' ')[0].replace(/px/, '')) || 2;
      tb.borderSpace = parseInt(this.IE ? tb.border || tb.currentStyle.borderLeftWidth :
        window.getComputedStyle(tb).borderLeftWidth.replace(/px/, '')) || 1;
      tb.extended = true;
      this.createGrips(th);
    };
